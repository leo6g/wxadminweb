...
public class ExportActSignData {
	public static void exportData(List<Map<String,Object>> data,
        ...
        String[] excelHeader = { "姓名","手机号码", "车型","报名地市名称","报名时间"};
        ...
        if(CollectionUtils.isNotEmpty(data)){
			...
			for (Map<String,Object> staffR2 : data) {
				...
				row.createCell(0).setCellValue(staffR2.get("name")==null?"":staffR2.get("name").toString());    
	            row.createCell(1).setCellValue(staffR2.get("phoneNumber")==null?"":staffR2.get("phoneNumber").toString());
	            row.createCell(2).setCellValue(staffR2.get("extStr1")==null?"":staffR2.get("extStr1").toString());
	            row.createCell(3).setCellValue(staffR2.get("belongCity")==null?"":staffR2.get("belongCity").toString()); 
...
//	            row.createCell(5).setCellValue(staffR2.get("statisDate")==null?"":staffR2.get("statisDate").toString()); 
		...
		response.setHeader("Content-Disposition", "attachment;fileName="+getFileName("活动报名数据表-"+DateUtil.getCurDate()));
