...
$(function () {
	...
	$("#saveBtn").click(function(){
		...
		var url = contextPath + "/wx/Resp/saveSubscribeReply";
		console.log(url);
		var params = {
			msgType : type,
			txtMsg : txtMsg,
		...
		Util.ajax.postJson(url,params,function(data){
			if(data.returnCode == 0) {
				alert("添加成功");
				C.load(contextPath + '/wx/Resp/subRespSet');
			} else {
	...
	$("#deleteBtn").click(function(){
		if(info == null){
			...
			alert("未设置自动回复",$.scojs_message.TYPE_ERROR);
			return;
		}
		confirm("确定删除吗?",function(){
		var url = contextPath + "/wx/Resp/deleteSubscribe";
