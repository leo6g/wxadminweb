...
var global_limit = 10 ;
	...
	$("#addBtn").click(function(){
		C.load(contextPath + "/biz/loan/add");
	});
	...
	$("#editBtn").click(function(){
		...
		C.load(contextPath + "/biz/loan/edit?fincId="+id);
	});
	...
	$("#viewBtn").click(function(){
		...
		var role = $("#role").val();
		C.load(contextPath + "/biz/loan/view?fincId="+id+"&role="+role);
	});
	...
	//动态改变广告及点赞状态
	Handlebars.registerHelper("tranExt",function(value){
        if(value=="T"){
       	  return "是";
	     }else if(value=="F"){
	          return "否";
	...
	Handlebars.registerHelper("tranLoan",function(value){
        if(value=="JY"){
       	  return "经营类";
	     }else if(value=="XF"){
	          return "消费类";
	...
	function getList(currentPage, limit){
		...
		var url = contextPath + "/biz/loan/getList";
		...
		Util.ajax.postJson(url, params, function(data, flag){
			...
			if(flag && data.returnCode=="0"){
				...
				$(".reviewBtn").click(function(){
					...
					var url = contextPath + "/biz/loan/reviewFinanceProd";
	...
	function delObj(id){
		confirm("确定删除吗？",function(){
			var url = contextPath + "/biz/loan/logicDelFinanceProd";
...
function fnPreview(tempId){
	$.ajax({
		...
		success : function(json) {
			var html = "<div style='position: relative;width: 580px;height: 200px;border-bottom:solid 2px black' onclick=fnViewArticle('"+json.bean.itemId+"')>" +
