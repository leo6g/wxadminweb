...
function addBIZAuthInfo(){
	...
	Util.ajax.postJson(url, params, function(data, flag){
		if(flag && data.returnCode=="0"){
				if(operFlag == "APPROVED"){
					if(role == 'infor'){
						updateLoanProdState("2-WAITING",prodId);
					}else if(role == "cardcheck"){
						updateLoanProdState("COMPLETED",prodId);
					}
				}else if(operFlag == "REJECTED"){
					if(role == "infor"){
						updateLoanProdState("1-REJECTED",prodId);
					}else if(role == "cardcheck"){
						updateLoanProdState("2-REJECTED",prodId);
					}
				}
		}else{
...
//修改信用卡表状态
function updateLoanProdState(authState,fincId){
